...
get_player_stats_team <- function(..., progress = TRUE, other = "") {
  ...
  get_player_stats_team_insist <- purrr::insistently(.get_player_stats_team,
                                                     rate = purrr::rate_delay(pause = 0.1, max_times = 10))

  get_team_player_stats <- function(team_url, team, league, season, ...) {

    tryCatch(get_player_stats_team_insist(team_url, team, league, season, ...),
  ...
  player_stats_team <- purrr::pmap_dfr(..., get_team_player_stats)
